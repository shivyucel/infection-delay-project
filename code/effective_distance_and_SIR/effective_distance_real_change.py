...
class EffectiveDistances:
    ...
    def load(self,fname, verbose=True, **kwargs):
        ...
        # Part deprecated in networkx 2.1
        # ccs = strongly_connected_component_subgraphs(G)
        # ccs = sorted(ccs, key=len, reverse=True)
        # G_GSCC = ccs[0] 

        # This part works for networkx 2.4 on
        ccs = strongly_connected_components(G)
        SCC = max(strongly_connected_components(G), key=len)
        G_GSCC = G.subgraph(SCC)
        ...
        for u, v, data in G.edges(data=True):
            traffic_reduction = self.merged.loc[self.merged.index == u]['marginal_change'].values[0]
            kappa   = (1.0-traffic_reduction)*kappa_0
    ...
    def get_dominant_path_distance(self, source=None, target=None, parameter=1, saveto=""):
        ...
        if source is None and target is None:
            #DPED = np.array([DPED_dic[s].values() for s in np.arange(self.nodes)]).transpose()
            DPED = np.array([ [ pair[1][i] for i in np.arange(self.nodes) ] for pair in DPED_dic ])
    ...
    def get_multiple_path_distance(self, source=None, target=None, parameter=1, cutoff=0, saveto="", verbose=False):
        ...
        if source is not None and target is not None:
            ...
            MPED = np.array([MPED_dic[s] for s in np.arange(self.nodes)]).transpose()
        else:
            if target is None:
                MPED = MPED_dic[source]
            else:
                MPED = np.array([MPED_dic[s][target] for s in np.arange(self.nodes)])
    ...
    def get_shortest_path_distance(self, source=None, target=None, saveto=""):
        ...
        if source is None and target is None:
            SPD = np.array([SPD_dic[s].values() for s in np.arange(self.nodes)]).transpose()
        elif (source is None) != (target is None):
...
if __name__ == "__main__":
    ...
    G = EffectiveDistances("line_graph.txt", verbose=False, dtype=int)
